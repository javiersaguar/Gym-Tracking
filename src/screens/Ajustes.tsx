  ...
  return (
    <div className="flex items-center gap-1 rounded-lg border border-line bg-paper p-1">
      <button
        ...
        className="pressable grid h-8 w-8 place-items-center rounded-md text-ink-faint transition-colors hover:bg-sunken hover:text-ink"
      >
...
function Switch({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    ...
    >
      <span
        className={cx(
          'absolute top-1/2 block h-5 w-5 -translate-y-1/2 rounded-full bg-paper shadow-card',
